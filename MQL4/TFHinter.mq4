...
void OnTick()
  {
   ...
   ObjectSetText("spreadLabel","Current Tick Spread: " + DoubleToStr(currentSpread), 10, "Arial", clrWhite);
      ...
      if(currentSpread >= maxPips && currentSpread > oldSpread)
        {
         ...
         if(currentSpread >= maxPips)
         {
             CreateDot(1.2);
             oldSpread = 0;
             printf(DoubleToStr(Time[1]));
         
         }
         
     }

   ObjectSetText("oldSpread","Highest Tick Spread: " + DoubleToStr(oldSpread), 10, "Arial", clrWhite);
  }
//+------------------------------------------------------------------+
//| ChartEvent function                                              |
//+------------------------------------------------------------------+
